...
4. Las cartas de jugador se perciben grandes (hasta 134 px de ancho en escritorio; `clamp(58px, 15.5vw, 82px)` en compacto), sin caja blanca exterior ni recortes laterales para 2–6 jugadores. Las etiquetas conservan nombre, monedas y color.
...
7. Cada carta y el mazo tienen una sombra negra sutil e independiente; el turno activo tiene un contorno blanco separado unos 9 px de la carta y un halo rojo exterior.
...
- 2026-09-26 05:06 -06:00: Nueva corrección de PR #10: se quitó la caja blanca exterior de los asientos; las cartas crecieron a 134px máximo en escritorio y 58–82px en compacto, con etiquetas de jugador en color. Se detectaron recortes compactos en los asientos próximos al borde para 3–5; `PlayerBoard.js` expone el borde derivado de la coordenada calculada y CSS compensa hacia adentro en <=520px, sin depender del índice ni alterar el cálculo base del círculo. Se regeneraron las 10 capturas; se revisaron 3p/6p escritorio y 3–6 compacto, con pares completos y sin solapamientos. Harness temporal de `App.js` restaurado; `start-pc` compiló antes y después, con advertencias ESLint existentes. `git diff --check` pasó. PR #10 sigue abierta, sin merge.
- 2026-09-26 06:58 -06:00: Revisión final de PR #10: el aro blanco se separó aproximadamente 9 px de la carta y conserva el halo rojo suave por fuera. En 6p compacto, los dos asientos inferiores subieron 32 px; en 5p se ajustó el margen lateral para evitar recortes. Se generó y revisó la matriz de 10 previews 2–6p escritorio/compacto, con inspección puntual de 3p y 6p; no se observan cruces entre cartas activas y laterales en 6p compacto. Harness temporal de `App.js` restaurado. El cliente compiló con advertencias ESLint existentes; no se ejecutaron tests automatizados. `git diff --check` pasó.

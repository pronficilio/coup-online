...
**Checkpoint F3:** F1–F3 `CLOSED`; estado global `WAITING_ORCHESTRATOR`. El tablero se verificó en vistas 2..6 y en turnos/pérdida/eliminación con eventos simulados. La acción por socket real no se recorrió y no se declara aprobada. La PR borrador [#7](https://github.com/pronficilio/coup-online/pull/7) está abierta a `master`; su head inicial `982c2b3de2ea188e2831f31d7fafac32f1c8af6f` recibió `PASS` del Verifier independiente. El próximo commit solo añade este checkpoint documental, así que se repetirá una comprobación del head actualizado. Mantener issue abierta; esperar autorización del usuario antes de integrar o cerrar.
